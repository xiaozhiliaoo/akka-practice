...
public class DistributedDataDocTest {
    ...
    public static void setup() {
        Config customConf = ConfigFactory.load("cluster.conf");
        system = ActorSystem.create("DistributedDataDocTest", customConf);
    }
    ...
    class DemonstrateGet extends AbstractActor {
        ...
        public Receive createReceive() {
            ...
            b.matchEquals(
                    ...
                    msg -> {
                        replicator.tell(
                                new Get<PNCounter>(counter1Key, Replicator.readLocal()), getSelf());

                        final ReadConsistency readFrom3 = new ReadFrom(3, Duration.ofSeconds(1));
                        replicator.tell(new Get<GSet<String>>(set1Key, readFrom3), getSelf());

                        final ReadConsistency readMajority = new ReadMajority(Duration.ofSeconds(5));
                        replicator.tell(new Get<ORSet<String>>(set2Key, readMajority), getSelf());

                        final ReadConsistency readAll = new ReadAll(Duration.ofSeconds(5));
                        replicator.tell(new Get<Flag>(activeFlagKey, readAll), getSelf());
                    });
            // #get
            ...
            return receiveBuilder()
                    .match(
                            ...
                            a -> {
                                ...
                                Optional<Object> reqContext = Optional.of(getSender());
                                replicator.tell(new Get<PNCounter>(counter1Key, readTwo), getSelf());
                            })
                    .match(
                            GetSuccess.class,
                            a -> a.key().equals(counter1Key),
